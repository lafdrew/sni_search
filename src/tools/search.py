...
from src.tools.searchapi_search.searchapi_search_results import SearchApiSearchResults
...
LoggedSearchApiSearch = create_logged_tool(SearchApiSearchResults)
...
def get_web_search_tool(max_search_results: int):
    ...
    elif SELECTED_SEARCH_ENGINE == SearchEngine.SEARCHAPI.value:
        engine = search_config.get("engine", "google")
        logger.info(
            f"SearchApi search configuration loaded: engine={engine}"
        )
        return LoggedSearchApiSearch(
            name="web_search",
            engine=engine,
            max_results=max_search_results,
        )
